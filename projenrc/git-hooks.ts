...
export class LintStaged extends Component {
  ...
  public addRule(pattern: string, command: string[] | string) {
    ...
    if (pattern in this.rules) {
      if (Array.isArray(this.rules[pattern]))
        (this.rules[pattern] as string[]).push(...command);
      else this.rules[pattern] = [this.rules[pattern] as string, ...command];
    } else this.rules[pattern] = command;
  }
